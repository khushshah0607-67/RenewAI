...
class PlantBase(BaseModel):
    ...
    def validate_export_limit(self):
        if (
            self.export_limit_mw is not None
            and self.export_limit_mw > self.installed_capacity_mw
        ):
            raise ValueError(
                "export limit cannot be greater than installed capacity."
            )
        return self
...
class PlantUpdate(BaseModel):
    ...
    def validate_export_limit(self):
        if (
            ...
            and self.export_limit_mw is not None
            and self.export_limit_mw > self.installed_capacity_mw
        ):
            raise ValueError(
                "export limit cannot be greater than installed capacity."
